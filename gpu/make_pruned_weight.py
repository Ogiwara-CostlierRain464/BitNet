...
def prune_w2_sparsity(ckpt_dir: str, output_path: str):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prune w2 weights in Transformer so as to not exceed 4096 nnz elements")
    parser.add_argument("--ckpt_dir", type=str, required=True, help="元の model_state_fp16.pt があるディレクトリ")
    parser.add_argument("--output", type=str, default="./checkpoints/pruned_randomly.pt", help="更新後のチェックポイントの保存先")
    args = parser.parse_args()

    prune_w2_sparsity(args.ckpt_dir, args.output)